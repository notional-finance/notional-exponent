...
abstract contract TestWithdrawRequest is Test {
    ...
    function checkCancelledWithdrawRequest(uint256 requestId, address staker) public virtual { }
    ...
    modifier approveVaultAndStakeTokens() {
        ...
        allowedDepositTokens[0].checkApprove(address(manager), allowedDepositTokens[0].balanceOf(address(this)));
        manager.stakeTokens(
            address(allowedDepositTokens[0]), allowedDepositTokens[0].balanceOf(address(this)), depositCallData
        );
        _;
    }
    ...
    function test_stakeTokens() public approveVault {
        for (uint256 i = 0; i < allowedDepositTokens.length; i++) {
            ...
            vm.prank(address(this));
            depositToken.checkApprove(address(manager), depositToken.balanceOf(address(this)));
            ...
            if (address(depositToken) != address(manager.YIELD_TOKEN())) {
                assertEq(
                    yieldTokensMinted,
                    finalYieldTokenBalance - initialYieldTokenBalance,
    ...
    function test_initiateWithdraw(bool partialWithdraw) public approveVaultAndStakeTokens {
        ...
        emit IWithdrawRequestManager.InitiateWithdrawRequest(
            address(this), address(this), initialYieldTokenBalance, sharesAmount, 0
        );
        ...
        (WithdrawRequest memory request, TokenizedWithdrawRequest memory tokenizedRequest) =
            manager.getWithdrawRequest(address(this), address(this));
        ...
        tokensWithdrawn = manager.finalizeAndRedeemWithdrawRequest(address(this), yieldTokenWithdraw, sharesToBurn);
    ...
    function test_initiateWithdraw_RevertIf_ExistingWithdrawRequest() public approveVaultAndStakeTokens {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ExistingWithdrawRequest.selector, address(this), address(this), requestId)
        );
        manager.initiateWithdraw(
            address(this), initialYieldTokenBalance, initialYieldTokenBalance, depositCallData, forceWithdrawFrom
        );
    }
    ...
    function test_initiateWithdraw_AfterFinalize() public approveVaultAndStakeTokens {
        ...
        yieldToken.checkApprove(address(manager), newYieldTokenBalance);
        manager.initiateWithdraw(
            address(this), newYieldTokenBalance, newYieldTokenBalance, withdrawCallData, forceWithdrawFrom
        );
    }
    ...
    function test_tokenizeWithdrawRequest_fullAmount(bool useManualFinalize) public approveVaultAndStakeTokens {
        ...
        uint256 requestId = manager.initiateWithdraw(
            address(this), initialYieldTokenBalance, initialYieldTokenBalance, withdrawCallData, forceWithdrawFrom
        );
        ...
        manager.finalizeAndRedeemWithdrawRequest(address(this), initialYieldTokenBalance, initialYieldTokenBalance);
    ...
    function test_tokenizeWithdrawRequest_TokenizeSameRequestTwice() public approveVaultAndStakeTokens {
        ...
        (WithdrawRequest memory request, TokenizedWithdrawRequest memory tokenizedRequest) =
            manager.getWithdrawRequest(address(this), addr1);
    ...
    function test_cancelWithdrawRequest_Success(bool multipleStakers) public approveVaultAndStakeTokens {
        ...
        yieldToken.checkApprove(address(manager), yieldToken.balanceOf(address(this)));
        uint256 initialYieldTokenBalance = yieldToken.balanceOf(address(this));
        if (multipleStakers) initialYieldTokenBalance = initialYieldTokenBalance / 2;
        uint256 sharesAmount = initialYieldTokenBalance / 2;

        uint256 requestId1 = manager.initiateWithdraw(
            staker1, initialYieldTokenBalance, sharesAmount, withdrawCallData, forceWithdrawFrom
        );
        uint256 requestId2 = 0;
        if (multipleStakers) {
            requestId2 = manager.initiateWithdraw(
        ...
        // The withdraw request should be removed from the account
        (WithdrawRequest memory request, TokenizedWithdrawRequest memory tokenizedRequest) =
            manager.getWithdrawRequest(address(this), staker1);
        assertEq(request.requestId, 0);
        assertEq(request.yieldTokenAmount, 0);
        assertEq(request.sharesAmount, 0);
        assertEq(tokenizedRequest.totalYieldTokenAmount, 0);
        ...
        // We should have received the yield tokens back.
        assertEq(yieldToken.balanceOf(address(this)), yieldTokensRefunded);

        checkCancelledWithdrawRequest(requestId1, staker1);
        ...
        if (multipleStakers) {
            yieldTokensRefunded = manager.cancelWithdrawRequest(staker2, bytes(""));
            assertEq(yieldTokensRefunded, initialYieldTokenBalance);

            // The withdraw request should be removed from the account
            (request, tokenizedRequest) = manager.getWithdrawRequest(address(this), staker2);
            assertEq(request.requestId, 0);
            assertEq(request.yieldTokenAmount, 0);
            assertEq(request.sharesAmount, 0);
            assertEq(tokenizedRequest.totalYieldTokenAmount, 0);
            ...
            // We should have received the yield tokens back.
            assertEq(yieldToken.balanceOf(address(this)), yieldTokensRefunded * 2);

            checkCancelledWithdrawRequest(requestId2, staker2);
        }
    }

    function test_cancelWithdrawRequest_Success_WithFinalization() public approveVaultAndStakeTokens {
        vm.skip(!canCancelWithdrawRequest);
        address staker1 = makeAddr("staker1");
        address staker2 = makeAddr("staker2");
        ...
        manager.finalizeRequestManual(address(this), staker2);
        (bool hasKnownAmount, uint256 amount) = manager.getKnownWithdrawTokenAmount(requestId2);

        uint256 tokensClaimed =
            manager.finalizeAndRedeemWithdrawRequest(staker2, initialYieldTokenBalance, sharesAmount);

        assertEq(tokensClaimed, ERC20(manager.WITHDRAW_TOKEN()).balanceOf(address(this)));
        if (hasKnownAmount) assertEq(amount, tokensClaimed);
