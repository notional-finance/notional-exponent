...
import { ICurvePool, ICurveRouterV2 } from "../../interfaces/Curve/ICurve.sol";
...
library CurveV2Adapter {
    ...
    struct CurveV2BatchData {
        ...
        // 7-11 for wrapped coin (underlying for lending or fake pool) -> LP token "exchange" (actually
        // `add_liquidity`),
        // 12-14 for LP token -> wrapped coin (underlying for lending pool) "exchange" (actually
        // `remove_liquidity_one_coin`) 15 for WETH -> ETH "exchange" (actually deposit/withdraw)
        ...
        returns (address spender, address target, uint256 msgValue, bytes memory executionCallData)
    {
        ...
        if (trade.tradeType == TradeType.EXACT_IN_SINGLE) {
            ...
            executionCallData = abi.encodeWithSelector(
                ICurvePool.exchange.selector, data.fromIndex, data.toIndex, trade.amount, trade.limit
            );
